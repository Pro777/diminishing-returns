...
We introduce the basic control concepts:
...
A key refinement: “decision” only counts if it’s paired with **owner + next action + deadline**.
...
We add a DR-like heuristic: if the group is repeating without adding **new options, evidence, or constraints**, we’re looping.

That gives us a third termination condition besides purpose/timebox.

### Round 4 — The tipping point (composite rule is stable)
By round 4, the conversation has effectively converged into a compact composite rule:
...
These rounds intentionally restate the already-stable rule to demonstrate what “diminishing returns” looks like.
